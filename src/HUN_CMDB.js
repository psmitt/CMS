...
function siteInventory(site_record, site_id, location_id) {
  ...
  runSQLQuery(myQuery(
    ...
    runSQLQuery(myQuery(query), result => {
      ...
      popup.onload = () => {
        if (Electron) {
          popup.document.getElementById('site').textContent = site[0][1]
          popup.document.getElementById('address').textContent = site[0][2]
        } else {
          popup.document.write(
        ...
        popup.document.body.lastElementChild.appendChild(tbody)
      }
    })
  })
}